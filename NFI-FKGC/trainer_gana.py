...
class Trainer:
    ...
    def build_theta(self):
        rel2id = json.load(open(self.data_path + '/relation2ids'))
        relation = list(rel2id.keys())
        rel_head = {rel2id[x]: [] for x in relation}
        rel_tail = {rel2id[x]: [] for x in relation}
        theta = {rel2id[x]: [] for x in relation}
        rel_counts = {rel2id[x]: 0 for x in relation}
        with open(self.data_path + '/path_graph') as f:
            lines = f.readlines()
            for tripls in lines:
                h, r, t = tripls.rstrip().split()
                rel_head[rel2id[r]].append(h)
                rel_tail[rel2id[r]].append(t)
                rel_counts[rel2id[r]] += 1
        # "heads per tail 每个尾巴有几个头" hpt = rel_counts[x]/(1+len(rel_tail[x]))
        # "tails per head 每个头有几个尾巴" tph = rel_counts[x]/(1+len(rel_head[x]))
        for x in relation:
            theta[rel2id[x]] = 1 / np.log(2 + rel_counts[rel2id[x]] / (1 + len(rel_tail[rel2id[x]])) + rel_counts[rel2id[x]] / (1 + len(rel_head[rel2id[x]])))
        self.theta = theta
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        if not iseval:
            ...
            p_score, n_score = self.metaR(task, iseval, curr_rel, support_meta, istest,self.theta)
